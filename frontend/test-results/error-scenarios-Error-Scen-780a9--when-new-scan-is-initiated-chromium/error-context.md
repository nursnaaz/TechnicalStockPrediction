...
- Name: error-scenarios.spec.ts >> Error Scenarios >> should clear previous errors when new scan is initiated
- Location: tests/e2e/error-scenarios.spec.ts:101:3
...
Locator: locator('[role="alert"]').first()
Expected: visible
Timeout: 5000ms
...
```yaml
- main:
  - heading "Bullish Stock Scanner" [level=1]
  - paragraph: Analyze technical indicators to identify potentially bullish stocks
  - textbox "Enter ticker symbols (e.g., AAPL, MSFT, GOOGL)"
  - button "Run Scan"
  - group: Please enter at least one ticker symbol
  - button
```
