...
class PatternDetector:
    ...
    def _detect_vcp(
        ...
        if len(vol_window) >= 40:
            ...
            vol_decline = (
                clamp01(1 - (second_half_vol / first_half_vol)) if first_half_vol > 0 else 0
            )
        else:
            vol_decline = 0.3
        ...
        confidence = clamp01(0.40 * (num_contractions / 3) + 0.30 * vol_decline + 0.30 * recency)
    ...
    def _detect_flat_base(
        ...
        for base_len in [60, 45, 30, 20, 80, 100]:
            ...
            confidence = clamp01(0.50 * tightness + 0.30 * vol_dryup + 0.20 * duration_factor)
    ...
    def _detect_tight_flag(
        ...
        for pole_end in range(len(prices) - 5, max(len(prices) - 25, 14), -1):
            ...
            confidence = clamp01(
                0.40 * pole_strength + 0.30 * retrace_quality + 0.30 * flag_compactness
            )
