...
async def scan(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid ticker list: {e!s}"
        )
    except Exception as e:
        logger.error(f"Unexpected error during scan: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Scan failed: {e!s}"
        )


@router.get("/scan/{scan_id}", response_model=ScanResponse, tags=["scan"])
async def get_scan(scan_id: str, scan_store: ScanStore = Depends(get_scan_store)):
    """
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to retrieve scan: {e!s}",
        )
