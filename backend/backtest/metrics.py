...
class BacktestMetrics:
    ...
    def calculate_aggregate_metrics(trades: list[dict]) -> dict:
        ...
        """
        if not trades:
            return {"total_trades": 0, "error": "No trades to analyze"}

        # Filter out trades without analysis
        analyzed_trades = [t for t in trades if t.get("status") == "analyzed"]
    ...
    def _analyze_by_score(trades: list[dict]) -> dict:
        ...
        """
        buckets = {"80-100": [], "70-79": [], "60-69": [], "50-59": [], "0-49": []}
        ...
        for bucket_name, bucket_trades in buckets.items():
            if not bucket_trades:
                result[bucket_name] = {"count": 0, "win_rate": 0, "avg_return": 0}
                continue
