...
class BacktestEngine:
    ...
    async def run_single_date_backtest(
        ...
        for ticker_score in scan_result.ranked_tickers:
            ...
            try:
                ...
                forward_data = await self._fetch_forward_data(ticker, as_of_date, calendar_days)
    ...
    async def _fetch_forward_data(self, ticker: str, start_date: str, days: int) -> list[dict]:
        """
        ...
        try:
            bars = await self.api_client.fetch_stock_data_range(
                ticker=ticker, from_date=from_date, to_date=end_date
            )
            return bars
    ...
    async def run_rolling_backtest(
        ...
        """
        logger.info(
            f"Starting rolling backtest: {start_date} to {end_date}, " f"frequency={frequency}"
        )
        ...
        for test_date in test_dates:
            result = await self.run_single_date_backtest(
                as_of_date=test_date, tickers=tickers, horizon_days=horizon_days
            )
    ...
    def _generate_test_dates(self, start_date: str, end_date: str, frequency: str) -> list[str]:
        """
