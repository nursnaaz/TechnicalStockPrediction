...
UNIVERSE = [
    'AAPL','MSFT','NVDA','GOOGL','GOOG','TSLA','AVGO','LLY','JNJ','UNH','PFE','ABBV','TMO','ABT',
    'DHR','MRK','BMY','AMGN','GILD','REGN','VRTX','ISRG','HD','COST','TGT','NKE','SBUX','MCD','CMG',
    'LULU','PG','KO','PEP','WMT','ORCL','ADBE','CRM','CSCO','INTC','AMD','QCOM','TXN','AMAT','LRCX',
    'KLAC','MRVL','BA','CAT','DE','UPS','HON','GE','LMT','RTX','XOM','CVX','COP','SLB','EOG','LIN',
    'APD','SHW','T','VZ','TMUS','V','MA','PYPL','SHOP','UBER','DDOG','CRWD','ZS','PANW','NET','MDB',
    'SNOW','NOW','TSM','IBM','CL','KMB','HSY','MDLZ','BKNG','PLD','AMT','EQIX','PSA','DLR','BBY',
    'SNPS','CDNS','NXPI','ADI','AMZN','META','ENPH','FSLR',
]
...
def features(price, ind, rs_pct):
    ...
    return {
        'score': None,
        'rsi': ind.rsi_14, 'roc': ind.roc_10, 'rs_raw': ind.relative_strength,
        'rs_pct': rs_pct, 'prox20d': ind.proximity_to_20d_high,
        'dist_sma50': dist_sma50, 'dist_sma200': dist_sma200,
        'macd_diff': (ind.macd_line - ind.macd_signal) if (ind.macd_line is not None and ind.macd_signal is not None) else None,
    }
...
async def main():
    ...
    for date in DATES:
        ...
        for t, data, ind in rows:
            ...
            if gain is None:
                continue
            f = features(price, ind, rs_pct); f['score'] = score; f['ticker'] = t; f['date'] = date; f['gain'] = gain
            (tps if gain >= 5 else fps).append(f)

    print(f"\nBUYs: {len(tps)+len(fps)}  TP={len(tps)}  FP={len(fps)}  "
          f"precision={len(tps)/(len(tps)+len(fps)):.1%}" if (tps or fps) else "no BUYs")
    ...
    for k in keys:
        tm, tmd = col(tps, k); fm, fmd = col(fps, k)
    ...
    for r in sorted(fps, key=lambda x: (-x['score'], x['gain']))[:12]:
        print(f"  {r['ticker']:<6} {r['date']} score={r['score']} gain={r['gain']:+.1f}% "
    ...
    rules = {
        ...
        "prox>=99 & rsi>=68 & dist50>=8":lambda r: g(r,'prox20d') >= 99 and g(r,'rsi') >= 68 and g(r,'dist_sma50') >= 8,
    ...
    for name, fn in rules.items():
        tp_rm = sum(1 for r in tps if fn(r)); fp_rm = sum(1 for r in fps if fn(r))
        kept_tp = len(tps) - tp_rm; kept_fp = len(fps) - fp_rm
