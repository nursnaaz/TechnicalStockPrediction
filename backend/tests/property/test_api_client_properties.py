...
class TestRetryLogicProperty:
    ...
    async def test_retry_on_different_error_types(
        self,
        error_type: str,
        max_retries: int
    ):
        ...
        **Validates: Requirements 1.4**
        ...
        with patch('core.api_client.config') as mock_config:
            ...
            with patch('asyncio.sleep', new_callable=AsyncMock) as mock_sleep:
                with pytest.raises(ApiError, match=f"Failed to fetch data for AAPL after {max_retries} retries"):
                    await client.fetch_stock_data("AAPL", days=50)
...
class TestSessionCachingProperty:
    ...
    @given(
        tickers=st.lists(
            st.text(alphabet=st.characters(whitelist_categories=['Lu', 'Nd']), min_size=1, max_size=10),
            min_size=1,
            max_size=10,
            unique=True
        ),
        days_values=st.lists(
            st.integers(min_value=10, max_value=365),
            min_size=1,
            max_size=5,
            unique=True
        ),
    ...
    async def test_cache_cleared_between_sessions(
        self,
        num_tickers: int,
    ...
    @settings(max_examples=20, deadline=5000)
    @given(
        ticker=st.text(alphabet=st.characters(whitelist_categories=['Lu', 'Nd']), min_size=1, max_size=10),
        days1=st.integers(min_value=10, max_value=365),
        days2=st.integers(min_value=10, max_value=365),
    )
    async def test_cache_key_includes_days_parameter(
        self,
        ticker: str,
        days1: int,
        days2: int
