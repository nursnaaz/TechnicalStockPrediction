...
valid_ticker_strategy = st.text(
    alphabet=st.sampled_from("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"), min_size=1, max_size=10
)
...
class TestTickerValidationProperties:
    ...
    @given(
        st.text(
            alphabet=st.characters(blacklist_characters="ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"),
            min_size=1,
        )
    )
...
class TestUniverseBuildingProperties:
    ...
    @settings(max_examples=20)
    @given(st.lists(st.one_of(valid_ticker_strategy, lowercase_ticker_strategy), min_size=1))
    ...
    @settings(max_examples=20)
    @given(
        st.lists(valid_ticker_strategy, min_size=1), st.lists(lowercase_ticker_strategy, min_size=1)
    )
