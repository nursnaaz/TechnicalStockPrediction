...
def ticker_score_strategy():
    ...
    return st.builds(
        ...
        indicators=st.fixed_dictionaries({
            'sma_50': st.floats(min_value=1.0, max_value=10000.0, allow_nan=False, allow_infinity=False),
            'ema_20': st.floats(min_value=1.0, max_value=10000.0, allow_nan=False, allow_infinity=False),
            'macd_line': st.floats(min_value=-100.0, max_value=100.0, allow_nan=False, allow_infinity=False),
            'macd_signal': st.floats(min_value=-100.0, max_value=100.0, allow_nan=False, allow_infinity=False),
            'macd_histogram': st.floats(min_value=-100.0, max_value=100.0, allow_nan=False, allow_infinity=False),
            'avg_volume_20': st.floats(min_value=1000.0, max_value=1000000000.0, allow_nan=False, allow_infinity=False),
            'relative_strength': st.floats(min_value=-100.0, max_value=100.0, allow_nan=False, allow_infinity=False)
        })
    )
...
class TestDescendingScoreSortWithCompletePreservation:
    ...
    def test_property_18_descending_score_sort(self, ticker_list):
        ...
        **Validates: Requirements 6.1, 6.2, 6.4**
        ...
        assert len(ranked_tickers) == len(ticker_list), \
        ...
        for i in range(len(ranked_tickers) - 1):
            assert ranked_tickers[i].bullish_score >= ranked_tickers[i + 1].bullish_score, \
        ...
        for i in range(len(ranked_tickers) - 1):
            if ranked_tickers[i].bullish_score == ranked_tickers[i + 1].bullish_score:
                ...
                if orig_pos_i is not None and orig_pos_i_plus_1 is not None:
                    assert orig_pos_i < orig_pos_i_plus_1, \
                        f"Stable sort violated: tickers with equal scores must maintain original order"
    ...
    def test_property_18_stable_sort_with_ties(self, ticker_list, duplicate_score):
        ...
        **Validates: Requirements 6.2**
        ...
        assert test_ticker_0_idx is not None and test_ticker_1_idx is not None, \
        ...
        assert test_ticker_0_idx < test_ticker_1_idx, \
            f"Stable sort violated: ticker at index 0 should appear before ticker at index 1 when scores are equal"
        ...
        **Validates: Requirements 6.4**
        ...
        if len(input_tickers) == len(ticker_list):  # All unique
            assert input_tickers == output_tickers, \
                "All input ticker symbols must appear in output"
    ...
    def test_property_18_all_equal_scores(self, score, count):
        ...
        **Validates: Requirements 6.1, 6.2, 6.4**
        ...
        assert all(t.bullish_score == score for t in ranked_tickers), \
            "All scores should be equal"
        
        # Order should be preserved
        for i in range(count):
            assert ranked_tickers[i].ticker == f"TICK{i}", \
    ...
    def test_property_18_descending_order_invariant(self, scores):
        ...
        **Validates: Requirements 6.1**
        ...
        assert ranked_scores == expected_sorted, \
            f"Ranked scores should match descending sort of input scores"
