...
from api.endpoints import get_orchestrator, get_scan, get_scan_store, health_check, scan
...
async def test_scan_successful_execution(
    sample_scan_request, sample_scan_response, mock_orchestrator, mock_scan_store
):
    """Test successful scan execution returns results and saves to store."""
    response = await scan(
        request=sample_scan_request, orchestrator=mock_orchestrator, scan_store=mock_scan_store
    )
    ...
    mock_orchestrator.execute_scan.assert_called_once_with(
        sample_scan_request, apply_signal_gate=True
    )

    # Verify result was saved
    mock_scan_store.save.assert_called_once_with(sample_scan_response.scan_id, sample_scan_response)
...
async def test_scan_handles_validation_error(
    sample_scan_request, mock_orchestrator, mock_scan_store
):
...
@pytest.mark.asyncio
async def test_scan_handles_scan_error(sample_scan_request, mock_orchestrator, mock_scan_store):
    """Test scan returns 500 for general scan errors."""
    # Mock orchestrator to raise ScanError without validation message
    mock_orchestrator.execute_scan.side_effect = ScanError("Failed to fetch market data")
...
@pytest.mark.asyncio
async def test_scan_handles_value_error(sample_scan_request, mock_orchestrator, mock_scan_store):
    """Test scan returns 400 for ValueError from universe builder."""
    # Mock orchestrator to raise ValueError
    mock_orchestrator.execute_scan.side_effect = ValueError("All tickers are invalid")
...
async def test_scan_handles_unexpected_error(
    ...
    mock_orchestrator.execute_scan.side_effect = RuntimeError("Unexpected database error")
...
@pytest.mark.asyncio
async def test_get_scan_successful_retrieval(sample_scan_response, mock_scan_store):
    """Test successful scan retrieval returns stored result."""
    scan_id = "test-uuid-1234"
    mock_scan_store.get.return_value = sample_scan_response

    response = await get_scan(scan_id=scan_id, scan_store=mock_scan_store)
...
async def test_get_scan_returns_404_when_not_found(mock_scan_store):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await get_scan(scan_id=scan_id, scan_store=mock_scan_store)
...
@pytest.mark.asyncio
async def test_scan_endpoint_validates_empty_ticker_list(mock_orchestrator, mock_scan_store):
...
@pytest.mark.asyncio
async def test_retrieve_flow(sample_scan_response, mock_scan_store):
