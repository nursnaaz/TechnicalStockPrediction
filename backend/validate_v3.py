...
UNIVERSE = [
    ...
    'TEAM','TSM','IBM','CL','KMB','HSY','MDLZ','CLX','BKNG',
    'PLD','AMT','EQIX','PSA','DLR','BBY','SNPS','CDNS','NXPI','ADI',
    'AMZN','META','F','GM','ENPH','FSLR',
]
...
async def run_dates(label, dates, tickers, horizon=30):
    ...
    for d in dates:
        res = await engine.run_single_date_backtest(as_of_date=d, tickers=tickers, horizon_days=horizon)
        if res.get('status') != 'completed':
            print(f"  {d}: {res.get('status')} {res.get('error','')}")
            continue
        analyzed = [t for t in res['trades'] if t.get('status') == 'analyzed' or 'predicted_bullish' in t]
        ...
        agg.extend(analyzed)
        print(f"  {d}: regime={regime:8s} trades={len(analyzed):3d} BUYs={buys:3d} "
              f"TP={tp} FP={fp} FN={fn} TN={tn} | P={prec:.0%} R={rec:.0%} F1={f1:.0%}")
    if agg:
        tp, fp, fn, tn, prec, rec, f1 = _confusion(agg)
        print(f"  ---- AGG: trades={len(agg)} TP={tp} FP={fp} FN={fn} TN={tn} "
